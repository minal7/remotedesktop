...
unsafe fn v6_socket_from_adapter(unicast_addr: &IpAdapterUnicastAddress) -> SocketAddrV6 {
    ...
    // VENDORED FIX (upstream webrtc-util 0.17.1 bug): read the address as
    // the raw 16 network-order bytes, not as `[u16; 8]`. The `Word()`
    // accessor returns the 8 u16 groups read in the host's native byte
    // order; on little-endian Windows that swaps the two bytes inside
    // every group, so `fe80::b774:…` came back as `80fe::74b7:…` — an
    // address that can't be bound, which silently killed every IPv6 ICE
    // candidate on Windows. `Byte()` is the same 16 bytes in wire order,
    // and `Ipv6Addr::from([u8; 16])` consumes them without reordering.
